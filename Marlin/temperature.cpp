    ...
    soft_pwm_amount[e] = (current_temperature[e] > minttemp[e] || is_preheating(e)) && current_temperature[e] < maxttemp[e] ? (int)get_pid_output(e) >> 1 : 0;
      ...
      if (watch_heater_next_ms[e] && ELAPSED(ms, watch_heater_next_ms[e])) { // Time to check this extruder?
        if (degHotend(e) < watch_target_temp[e])  {
          ...
          TFT_SERIAL_ENTER(); // SEND MESSAGE TO TFT
  ...
  void Temperature::thermal_runaway_protection(Temperature::TRState * const state, millis_t * const timer, const float &current, const float &target, const int8_t heater_id, const uint16_t period_seconds, const uint16_t hysteresis_degc) {
    ...
    */
    ...
    switch (*state) {
      ...
      case TRFirstHeating:
        if (current >= (tr_target_temperature[heater_index]- (WATCH_TEMP_INCREASE + TEMP_HYSTERESIS + 1))){watch_heater_next_ms[HOTEND_INDEX] = 0;}
...
void Temperature::readings_ready() {
  ...
  static char hotendTempCounter=0;
  ...
  for (uint8_t e = 0; e < COUNT(temp_dir); e++) {
    ...
    if (current_temperature_bed_raw GEBED bed_maxttemp_raw && target_temperature_bed > 0.0f) {
        ...
        if(BedMinTempCounter>20){
            BedMinTempCounter=0;
            errorFlag=5;max_temp_error(-1);
        }
      } else if (bed_minttemp_raw GEBED current_temperature_bed_raw && target_temperature_bed > 0.0f) {
        BedMinTempCounter++;
        if(BedMinTempCounter>20) {
            BedMinTempCounter=0;
            errorFlag=3;min_temp_error(-1);
        }
      } else {
       BedMinTempCounter=0;
      }
