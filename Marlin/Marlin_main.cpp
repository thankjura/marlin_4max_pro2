              ...
              if((!planner.movesplanned())&&(!TFTresumingflag)) {
                ...
                if(TFTcode_seen('X')) {
                  ...
                  if((coorvalue<=0.2)&&coorvalue>0){
                    ...
                    sprintf_P(value,PSTR("G1 X%iF%i"),int(coorvalue),movespeed); enqueue_and_echo_command_now(value); 
                  }
...
void TFT_Commond_Scan() {
 ...
 static unsigned int Scancount=0;
  ...
  if(Scancount>16000){ errorFlag = 7;Scancount=0;NEW_SERIAL_PROTOCOLPGM("J10");TFT_SERIAL_ENTER();}//T0 unnormal//2019.4.23
}
