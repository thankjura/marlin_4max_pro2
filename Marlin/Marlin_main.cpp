          ...
          switch((int)((strtod(&TFTcmdbuffer[TFTbufindw][TFTstrchr_pointer - TFTcmdbuffer[TFTbufindw] + 1], NULL)))) {
            ...
            case 7://A7 GET PRINTING TIME
              ...
              if(starttime != 0)  {
                ...
                uint16_t time = millis()/60000 - starttime/60000;
                NEW_SERIAL_PROTOCOL(i8tostr3(time/60));
                TFT_SERIAL_SPACE();
            ...
            case 15://A15 RESUMING FROM OUTAGE
              ...
              if(!planner.movesplanned() && !TFTresumingflag) {
                ...
                current_position[Z_AXIS]=settings.last_position[1];      //2019.6.17 Daxiog
                ResumingFlag=1;
                card.startFileprint();
                starttime=millis();
            ...
            case 18://a18 set fan speed
              unsigned int myTemp;
              if (TFTcode_seen('S')) {
                myTemp = (TFTcode_value()*Max_ModelCooling/100);
                myTemp  =constrain(myTemp,0,Max_ModelCooling);
                //  myTemp=(TFTcode_value()*179/100);
                //  myTemp=constrain(myTemp,0,179);
                fanSpeeds[0] = myTemp;
              } else {
              ...
              if(!planner.movesplanned() && !TFTresumingflag) {
                ...
                if (TFTcode_seen('X')) {
                  ...
                  if (coorvalue<=0.2 && coorvalue > 0){
                    sprintf_P(value, PSTR("G1 X0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else if (coorvalue <= -0.1 && coorvalue >-1) {
                    sprintf_P(value, PSTR("G1 X-0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else {
                    sprintf_P(value, PSTR("G1 X%i F%i"), int(coorvalue), movespeed);
                    enqueue_and_echo_command_now(value); 
                  }
                } else if(TFTcode_seen('Y')) {
                  coorvalue=TFTcode_value();
                  if (coorvalue <= 0.2 && coorvalue > 0) {
                    sprintf_P(value, PSTR("G1 Y0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else if (coorvalue <= -0.1 && coorvalue>-1) { 
                    sprintf_P(value, PSTR("G1 Y-0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else {
                    sprintf_P(value, PSTR("G1 Y%i F%i"), int(coorvalue), movespeed);
                    enqueue_and_echo_command_now(value); 
                  }
                } else if(TFTcode_seen('Z')) {
                  coorvalue=TFTcode_value();
                  if(coorvalue <= 0.2 && coorvalue > 0) {
                    sprintf_P(value, PSTR("G1 Z0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else if(coorvalue <= -0.1 && coorvalue>-1) {
                    sprintf_P(value, PSTR("G1 Z-0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else {
                    sprintf_P(value, PSTR("G1 Z%i F%i"), int(coorvalue), movespeed);
                    enqueue_and_echo_command_now(value); 
                  }
                } else if (TFTcode_seen('E')) {
                  coorvalue=TFTcode_value();
                  if (coorvalue <= 0.2 && coorvalue > 0) {
                    sprintf_P(value, PSTR("G1 E0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else if (coorvalue <=-0.1 && coorvalue >-1 ){
                    sprintf_P(value, PSTR("G1 E-0.1 F%i"), movespeed);
                    enqueue_and_echo_command_now(value);
                  } else {
                    sprintf_P(value, PSTR("G1 E%i F500"), int(coorvalue));
                    enqueue_and_echo_command_now(value);
                  }
                }
                enqueue_and_echo_commands_P(PSTR("G90"));
            ...
            case 30:
              ...
              } else if(TFTcode_seen('I')) {
                enqueue_and_echo_commands_P(PSTR("G1 Z5 F500"));
                enqueue_and_echo_commands_P(PSTR("G1 X240 Y30 F5000"));
                enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));
              } else if(TFTcode_seen('J')) {
                enqueue_and_echo_commands_P(PSTR("G1 Z5 F500"));
                enqueue_and_echo_commands_P(PSTR("G1 X240 Y190 F5000"));
                enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));
              } else if(TFTcode_seen('K')) {
                enqueue_and_echo_commands_P(PSTR("G1 Z5 F500"));
                enqueue_and_echo_commands_P(PSTR("G1 X30 Y190 F5000"));
                enqueue_and_echo_commands_P(PSTR("G1 Z0 F300"));
              } else if(TFTcode_seen('L')) {
                enqueue_and_echo_commands_P(PSTR("G1 X130 Y100  Z50 F5000"));
              }
              break;
            case 31: //a31 zoffset
              /*
            ...
            case 33:// a33 get version info
              if(errorFlag==0) {
                ...
                NEW_SERIAL_PROTOCOLPGM("J33 ");
                NEW_SERIAL_PROTOCOLPGM("ReadSD error!");
                TFT_SERIAL_ENTER();
                ...
                NEW_SERIAL_PROTOCOLPGM("J33 ");
                NEW_SERIAL_PROTOCOLPGM("T0Drop");
                TFT_SERIAL_ENTER();
              } else if(errorFlag==7) {
                NEW_SERIAL_PROTOCOLPGM("J33 ");
                NEW_SERIAL_PROTOCOLPGM("outRange");
                TFT_SERIAL_ENTER();
                printf("outRange\n");
              } else if(errorFlag==8) {
                NEW_SERIAL_PROTOCOLPGM("J33 ");
                NEW_SERIAL_PROTOCOLPGM("SdLost");
                TFT_SERIAL_ENTER();
                printf("SdLost\n");
              } else if(errorFlag==9) {
                NEW_SERIAL_PROTOCOLPGM("J33 ");
                NEW_SERIAL_PROTOCOLPGM("ReadError");
                TFT_SERIAL_ENTER();
                printf("Sd read error\n");
              }
              break;
            case 40://a40 reset mainboard
              NEW_SERIAL_PROTOCOLPGM("J17 ");
              TFT_SERIAL_ENTER();
              delay(100);
              softwareReset();
              break;
            ...
            case 42://a42 light
              ...
              //sprintf(myi2c.I2C_SRBUF,"A100T55555");
              // myi2c.TestMyI2C_(i2c,myi2c.addr,0);
              //myi2c.resetbuf();
...
void setupMyZoffset() {
  (void)settings.ReadMyZoffset();
  (void)settings.readFirstBootFlag();
  if(settings.FirstBootFlag!=0x5a) {
    settings.FirstBootFlag=0x5a;
    (void)settings.SaveFirstBootFlag();
    Current_z_offset=0.2;
    (void)settings.SaveMyZoffset();
  } else {
    Current_z_offset=settings.last_z_offset;
  }
}
...
inline void get_serial_commands() {
  ...
  inline void get_sdcard_commands() {
    static char sdReadErrorCounter = 0;
    ...
    while (commands_in_queue < BUFSIZE && !card_eof && !stop_buffering&&!seekdataflag) {
      ...
      if (card_eof || n == -1
        ...
        if (card_eof) {
          ...
          errorFlag=1;
		   sdReadErrorCounter ++;
		   if( sdReadErrorCounter>20)
		   	{
		   	 sdReadErrorCounter = 0;
		   	 errorFlag=9;
			 kill(PSTR("SD read error"));
		   	}
        }
		else if(n!=-1)
		{
    ...
    destination[E_AXIS]=settings.last_position[0];
    current_position[E_AXIS]=settings.last_position[0];
	planner.set_e_position_mm(settings.last_position[0]);
    planner.buffer_line (X_MIN_POS,Y_MIN_POS,settings.last_position[1],settings.last_position[0],feedrate_percentage,active_extruder);
		destination[X_AXIS]=settings.last_position[3]; //SET A NEW ORIGNAL COORDINATE
		 destination[Y_AXIS]=settings.last_position[2];
		 //if(current_position[Z_AXIS]>0.3) current_position[Z_AXIS]=settings.last_position[1]-0.1;
		 //else
		 	current_position[Z_AXIS]=settings.last_position[1];      //2019.6.17 Daxiog
...
//#if ENABLED(BEZIER_CURVE_SUPPORT)
  ...
  inline void gcode_G5() {
      ...
      if(FlagResumFromOutage) {
        ...
        card.setIndex(settings.last_sd_position[0]);
        seekdataflag=1;
        ResumingFlag=1;
...
//#endif // BEZIER_CURVE_SUPPORT
  ...
  void log_machine_info() {
    ...
    SERIAL_ECHOPGM("Probe: ");
      ...
      SERIAL_ECHOPAIR(" Z:", zprobe_zoffset);7
...
inline void gcode_G28(const bool always_home_all) {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    if (FlagResumFromOutage==1 && homeZ ) {
        SERIAL_PROTOCOLLN("forbit home_Z");
        return ;
    }
    ...
    if (z_homing_height && (home_all || homeX || homeY) && FlagResumFromOutage==0) {
      ...
      if (destination[Z_AXIS] > current_position[Z_AXIS]) {
        ...
        SERIAL_PROTOCOLLN("home_Z_Step:1");
      }
    }
      ...
      SERIAL_PROTOCOLLN("home_XY_Step:1");
    ...
    if (home_all || homeX
        ...
        homeaxis(X_AXIS);
        SERIAL_PROTOCOLLN("home_X_Step:1");
    ...
    // Home Y (after X)
      ...
      SERIAL_PROTOCOLLN("home_Y_Step:1");
    #endif
...
inline void gcode_M105() {
  ...
  inline void gcode_M106() {
    ...
    if (p < FAN_COUNT) {
      ...
      const uint16_t s = parser.ushortval('S', 179);      
...
inline void gcode_M226() {
  ...
  inline void gcode_M300() {
    int beepS = parser.seen('S') ? parser.value_int() : 110;
    int beepP = parser.seen('P') ? parser.value_int() : 1000;
    if (beepS > 0) {
      #if BEEPER_PIN > 0
        tone(BEEPER_PIN, beepS);
        delay(beepP);
...
void TestLine() {
    ...
    if(commands_in_queue< BUFSIZE) {
        if(xxx==0) {
            xxx++;
            enqueue_and_echo_commands_P(PSTR("G1 Y200 F5000"));  //
        } else {
            xxx = 0;
            enqueue_and_echo_commands_P(PSTR("G1 Y5 F5000"));  //
        }
    }
}
